...
int8_t Null_ptr;
int8_t flag_empty_ut;



//Initializing the pointer for Circular buffer by allocating memory.
BUFFER_STATUS Init_Buffer(Buffer_Parameters *Buffer_Ptr, uint32_t Buffer_Length)

{
	...
	else
	{
		...
		Buffer_Ptr->Buffer_Base_Pointer=(uint32_t*)pvPortMalloc(Buffer_Length*(sizeof(char)));
...
BUFFER_STATUS Add_Element_To_Buffer(Buffer_Parameters *Buffer_Ptr, uint32_t Value)
		...
		else
		{
			...
			if(Buffer_Ptr->Buffer_Tail==Buffer_Ptr->Buffer_Base_Pointer)
			{
				...
				*(Buffer_Ptr->Buffer_Tail)=Value;
				...
				(Buffer_Ptr->Buffer_Head)=(Buffer_Ptr->Buffer_Head)+1;
...
BUFFER_STATUS Expand_Buffer(Buffer_Parameters *Buffer_Ptr, int8_t Buffer_Length)
{
	...
	Buffer_Ptr->Buffer_Base_Pointer=(uint32_t*)realloc(Buffer_Ptr->Buffer_Base_Pointer,Buffer_Length);
